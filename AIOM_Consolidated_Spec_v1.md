...
Discussion (self-explanation register): why a per-seat contract does not make AI a seat-priced good; what the Cursor and Copilot episodes reveal about where the meter was all along; construct the strongest version of the flat-rate objection yourself, then answer it.
Problems (fully worked): (P1, worked) model CIO memo reply per assessment 1, with annotated reasoning; (P2, worked) consumption-event inventory on a second cited deployment; (P3, completion) inventory with the event-type column blank. Interleaving: none yet (first chapter).
