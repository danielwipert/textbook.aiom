...
In April 2026, four months into the calendar year, Uber had spent its entire annual budget for artificial intelligence. Nothing had broken. No contract had been renegotiated and no vendor had raised a price. The engineers were doing the work the company had asked them to do, with the tool the company had given them.
...
That is a successful deployment by every measure an engineering organization would normally apply. The tool was adopted, it was used heavily, and by spring a large majority of the code being committed had passed through it. Adoption is usually the hard part. Here it was the easy part.

The finance team had a number for what this would cost. The number came from the same place such numbers usually come from: a price per engineer, multiplied by a count of engineers, held roughly flat across the year. That is how software has been budgeted for thirty years, and for software it works.
...
Consider what the company could have known in February. Adoption had reached about a third of engineers. If anyone had multiplied the observed cost per engineer by the adoption trend, the April date would have been visible two months ahead. The information existed. The comparison did not.

Two things about this episode are worth separating, because they are usually confused. The first is that the company could see what it was spending. The invoices arrived, the totals were correct, and by April the arithmetic was unambiguous. The second is that the company could not say what it had received. The chief operating officer was publicly asking whether the spending had been worth it, which is not a question an executive asks about a line item whose return is documented.

This is the position Chapter 1 predicted and did not describe. Uber had not made a category error. It knew perfectly well that it was buying a metered resource. It had simply never built the thing that would let it manage one.
...
An unmanaged usage flow grows toward whatever the tool makes easy. This is not a criticism of the people using it. An assistant that drafts code well is used more than one that drafts code badly, and the organization wanted it used. Growth in the usage flow is the deployment succeeding.
...
Uber’s position in April 2026 is the first error in progress. The company knew its cost to the dollar. Its chief operating officer was asking, in public, what the company had received. Both statements were true at once, and they were true because of how the two numbers were produced rather than because of anything the engineers had done.
...
Run against Uber as reported, the mapping produces a clear result. The usage flow is managed and healthy: adoption was tracked by month and the direction was known. The record flow is partly managed: a per-engineer cost range was reported, so consumption was visible at some grain, but it did not reach the forecast that finance was planning against. The cost-and-value flow is unmanaged on the value half, which is what the chief operating officer’s question reveals. One flow of three was unmanaged, and it was enough.
...
Work through the flows in order. The usage flow is managed: adoption was tracked month by month, the direction was known, and the organization could state what share of engineers were using the tool. Nothing about this flow was a surprise.

The record flow is partly managed. A per-engineer monthly cost range was reported, so consumption was visible at some grain. But the forecast that finance planned against held cost flat per engineer, which means the record that existed did not reach the place where the decision was made. The evidence that would change this diagnosis is a monthly consumption figure attributed to a team and compared against a plan.
...
The diagnosis is therefore one flow healthy, one partial, one half-built, and the organization ran out of budget anyway. This is the point of the mapping: a deployment does not need to be failing on all three flows to lose economic control.
