...
Filed in the step folder rather than at the repo root because promoting it to
repo tooling is still unruled (HANDOFF thread 0). It is kept here on the
`apply_round1.py` precedent: the record of how a step's artifact was produced,
so the step is reproducible without the script having to be rewritten from
scratch a third time.

The Stage 3 packet of 2026-08-10 was generated by a throwaway script in a session
scratchpad and did not survive its container. This is the second time the same
work has been needed on the same chapter, and fifteen chapters each need it twice,
at Stage 3 and again at Stage 7.

Usage:
    python3 build_factcheck_packet.py <chapter.html> --stage 7 --render <name.pdf> --out <out.md>
"""
...
def main():
    ...
    out.append("Generated from the live text by `build_factcheck_packet.py`, filed beside")
    out.append("this file. Stage 7 is structurally external: no source host is reachable")
    out.append("from the Claude environment, verified 2026-08-06 against six of them, so")
    out.append("nothing below is a verification. It is the material a checker needs.\n")
