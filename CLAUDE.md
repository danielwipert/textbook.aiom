...
| `prose_extract.py` | Chapter HTML to reviewer-readable prose, for a second-model package. Promoted to the root 2026-08-28 on the SAME reasoning as `factcheck_packet.py`, at three times rather than twice: the bias check, Stage 1 and Stage 2 each rewrote it. **The throwaway versions were wrong in ways nobody could see from the output**: one invented a `[TEACHING BODY]` slot label neither chapter carries, because the skeleton signposts through structure rather than labels, and dropped every key term's NAME. Keeps term names, marks each figure in place with its caption, renders the theorem panel as a structured conditional and the dated boxes as boxes, and excludes the source register and the citation markers. **Verified by comparing its output's word multiset against the chapter's own**, against both chapters, after every change. Not a gate and nothing depends on it. |
