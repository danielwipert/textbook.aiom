...
1. **Chapter 1 Stage 7 IS NEXT AND IS DAN'S, then Stage 8. STAGE 6 IS CLOSED at
   9 of 13.** Final fact check 2, then final read. **THE STAGE 7 PACKET IS BUILT
   AND WAITING**, in `09_Stage7_Final_Fact_Check_2/`: the 25-page render with all
   fourteen gates green, and `AIOM_Ch1_Stage7_Claim_Inventory.md` pairing all six
   cited passages with the register entry behind each key, notes verbatim. The
   mechanical checks are banked on its first page and need not be repeated.

   **STAGE 7 IS STRUCTURALLY EXTERNAL**, not external by preference. No source
   host is reachable from the Claude environment, verified 2026-08-06 against six
   of them. Claude can rule on whether prose stays inside what a register note
   says. Claude cannot verify the note against the source, and must not offer to.

   **SF11 WAS RAISED AND RULED IN THIS SESSION, and it is the FIFTH instance of a
   record outliving the prose it describes.** The `altman-2025-pro` note recorded
   that the sentence "now reads 'OpenAI's chief executive, Sam Altman, stated
   publicly'". It had not since 2026-08-08, when the round-1 copy-edit return
   rewrote it, the same round that produced SF8 to SF10 and orphaned the note
   behind SF7. THE SF1 RULING WAS UNAFFECTED and the prose was not changed: SF1
   banned a superlative and required the firm be named, and the current sentence
   satisfies both. Only the note's description was stale. Fixed on the A2
   precedent, appending the drift and the current wording beneath the dated line
   rather than editing it.

   **A REFINEMENT TO THE CE2 RULE, MEASURED RATHER THAN REASONED.** CE2 said an
   edit to the live text invalidates the outstanding export. SF11 edits the live
   text and does not: round 5 still resolves 209 of 221 spans. The register sits
   after every exported block and is excluded from the export, so a register-only
   edit shifts no body span. The rule is narrower than CE2 stated: an edit
   invalidates the export when it falls at or before an exported block, which
   every prose edit does and a register-only edit does not.

   Historical, from when Stage 6 was still open: **CE1 IS APPLIED
