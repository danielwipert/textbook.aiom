...
| ~~G-J~~ | ~~**WeasyPrint 69 can render a footnote pages away from its call.**~~ **CLOSED 2026-08-08, and the first version of this entry was WRONG.** It recorded the defect as structural and unfixable on the strength of four prose perturbations that moved the chapter's text without moving either failure. The conclusion did not follow: prose length was simply the wrong lever. `widows` and `orphans` move it. All nine combinations of 2, 3 and 4 were measured against the fourteen gates, and four of them clear gate 8 outright. CSS v7.0 sets `widows: 2; orphans: 3`, which is the only green pair that also SHORTENS the chapter, 26 pages to 25, and it removes the worst short page (Chapter 1 page 11, which had been ending at 428pt against a normal 626pt). WHAT REMAINS TRUE: the underlying renderer behaviour is not fixed and is not understood. It does not reproduce in a minimal document; a harness sweeping filler length, container height and `break-inside` in both configurations is clean every time. `footnote-policy: line` is load-bearing and must stay, since removing it adds a failure while `footnote-policy: block` changes nothing. THE WIDOWS SETTING IS GOOD TYPOGRAPHY ON ITS OWN MERITS AND WAS RULED ON THAT BASIS; clearing gate 8 is a side effect of repagination and MUST NOT be expected to repeat on Chapters 2 to 15. If gate 8 fails on a later chapter, this entry is where the diagnosis starts, not a settled fix. | resolved for Ch1, renderer behaviour still open |
| ~~G-K~~ | ~~**Gate 14 cannot tell a one-line paragraph from a widow.**~~ **CLOSED 2026-08-08, the same day it was opened.** `qa()` now takes `source_html` and gate 14 builds the set of complete body paragraphs from it. A rendered line whose ENTIRE text is a complete source paragraph cannot be the last line of a longer one, so it is neither a widow nor an orphan. The discriminator has to come from the source because it is not in the render: from the PDF alone a one-line paragraph and a widow are identical. The obvious PDF-only test, whether the previous page's last line fills the measure, was tried and does not work, because text flowing beside a floated definition callout is legitimately narrow and a mid-paragraph line at a page break can measure 53 percent (Chapter 1 page 12 does exactly that). Verified by control: withholding `source_html` restores the phantom, and the real widow on page 13 is still reported either way. `place.py` passes it too. Original text: gate 14 counted a one-line paragraph at the head of a page as a widow; the 2026-08-06 key-term amendment keyed on the semibold face and could not help ordinary body prose. | resolved | The 2026-08-06 amendment closed this for key-term names by excluding a line that is entirely semibold, which works only because a term name has a font signal. A one-line paragraph in ORDINARY body prose has none, and the first one on a page still scores as a widow. Found 2026-08-08, when the Q1 ruling set section 1.4's five diagnostic questions as five short standalone paragraphs: "Where is the resource being used?" opens page 14 and gate 14 reports it as a widow. It is a whole paragraph, not the last line of one. The obvious discriminator, whether the previous page's last line filled the measure, does not work: `width()` sums glyph widths and excludes inter-word spaces, so a justified mid-paragraph line at a page break measures short too. | **Medium.** One of Chapter 1's two reported widows is this phantom; the other, on page 13, is real. Until closed, a widow count must be read against the pages, not trusted. |
